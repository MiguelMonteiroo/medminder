...
MedMinder declares `USE_FULL_SCREEN_INTENT` because a core app function is presenting a time-sensitive medication alarm over the lockscreen. Complete the Full-Screen Intent declaration after uploading the first signed AAB to an internal-test draft and before publishing that test release. Do not wait for production submission. Describe:
